...
const log = createLogger("sweep-charges");
...
/**
 * Finds all `charge` ledger entries that have not yet been swept.
 * A charge is considered swept when a `charge_sweep` entry with a `tx_hash`
 * exists for the same wallet. Only wallets whose aggregate meets the sweep
 * threshold are returned.
 */
const findSweepableWallets = async (): Promise<WalletChargeAggregate[]> => {
  const swept = db
    .select({ walletId: walletLedgerEntries.walletId })
    .from(walletLedgerEntries)
    .where(
      and(
        eq(walletLedgerEntries.kind, "charge_sweep"),
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(walletLedgerEntries.kind, "charge"),
        eq(walletLedgerEntries.networkEnv, config.networkEnv),
  ...
  const results: WalletChargeAggregate[] = [];
...
/**
 * Records a `charge_sweep` ledger entry for the wallet with the on-chain tx hash.
 * The existence of this row (with a non-null tx_hash) is what marks the
 * wallet's charge entries as swept.
 */
const insertChargeSweepEntry = async ({
  walletId,
  amountNative,
  txHash,
  targetAddress,
 ...
 * charges wallet. The sponsor (wallet-indexer) pays for gas.
 */
 ...
 */
export const sweepAccumulatedCharges = async (): Promise<SweepResult> => {
  const startedAt = performance.now();

  const chargesWallet = config.chargesWallet;
  if (!chargesWallet) {
    log.debug("sweep-charges skipped: CHARGES_WALLET not configured");
  ...
  let sweepableWallets: WalletChargeAggregate[];

  try {
    sweepableWallets = await findSweepableWallets();
  } catch (error) {
    const details = serializeUnknownError(error);
    log.error("charge sweep failed to query sweepable wallets", {
  ...
  if (sweepableWallets.length === 0) {
    log.debug("charge sweep completed: no wallets above threshold");
    return { totalSwept: 0n, walletsProcessed: 0, errors: 0 };
  }
  ...
  for (const wallet of sweepableWallets) {
    ...
    log.info("sweeping wallet charges", walletLogCtx);
    ...
    try {
      ...
      const txHash = await executeSponsoredTransfer({
        ...
        sponsorKeypair,
        sponsorAddress,
        targetAddress: chargesWallet,
        amount: wallet.totalNative,
      });

      log.info("charge sweep transfer confirmed", { ...walletLogCtx, txHash });

      await insertChargeSweepEntry({
        walletId: wallet.walletId,
      ...
      log.info("charge_sweep entry recorded", {
        walletId: wallet.walletId,
        txHash,
      ...
      errors += 1;
      const details = serializeUnknownError(error);
      log.error("charge sweep failed for wallet", {
        ...walletLogCtx,
        error: details.message ?? details.summary,
        errorDetails: details,
      });
  ...
  log.info("charge sweep completed", {
